...
function send(mode) {
    try {
        ...
        $.ajax({
            type: "POST",
            url: "http://www.tnaskills.com/execute/User.ashx",
            ...
            success: function (text) {
                ...
                for (i = 0; i < obj.rows.length; i++) {
                    $('<li >').append('<a href="javascript:list(\'' + obj.rows[i].UserID + '\',\'' + obj.rows[i].UserName + '\',\'' + obj.rows[i].phone + '\',\'' + obj.rows[i].email + '\',\'' + obj.rows[i].password + '\')" class="ui-btn ui-btn-icon-left  ui-icon-minus"> ' + obj.rows[i].UserName + '</a> ').appendTo('#alldata');
                }
