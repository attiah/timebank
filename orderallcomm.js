...
function send(mode) {
    try {
        ...
        $.ajax({
            type: "POST",
            url: "http://www.tnaskills.com/execute/orderallcomm.ashx",
            data: { id: id },
            success: function (text) {
                ...
                for (i = 0; i < obj.rows.length; i++) {
                    $('<li >').append('<a href="#" class="ui-btn ui-btn-icon-left  ui-icon-minus"><h2> ' + obj.rows[i].commname + '</h2><h2>' + obj.rows[i].GDate + '</h2><h2>' + obj.rows[i].text + '</h2></a> ').appendTo('#alldata');
                }
